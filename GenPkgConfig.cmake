...
function(configure_pkg_config_file TARGET)
	cmake_parse_arguments(""
		"" # options
		"NAME;VERSION;DESCRIPTION;URL;COMPONENT;INSTALL_LIB_DIR;INSTALL_INCLUDE_DIR;PKGCONFIG_DIR" # one_value_keywords
		"REQUIRES;CONFLICTS" # multi_value_keywords
		${ARGN}
	)

	configure_pkg_config_file_vars("${TARGET}" "${_NAME}" "${_INSTALL_LIB_DIR}" "${_INSTALL_INCLUDE_DIR}" "${_COMPONENT}" "${_DESCRIPTION}" "${_URL}" "${_VERSION}" "${_REQUIRES}" "${_CONFLICTS}" "${_PKGCONFIG_DIR}")
endfunction()
...
function(configure_pkg_config_file_vars TARGET _NAME _INSTALL_LIB_DIR _INSTALL_INCLUDE_DIR _COMPONENT _DESCRIPTION _URL _VERSION _REQUIRES _CONFLICTS _PKGCONFIG_DIR)
	...
	if(_DESCRIPTION)
		...
		message(WARNING "pkg-config requires a description! A dummy one is set.")
		set(_DESCRIPTION "No description provided")
	endif()
	...
	if(_PKGCONFIG_DIR STREQUAL "")
		set(_PKGCONFIG_DIR "${_INSTALL_LIB_DIR}/pkgconfig")
	endif()
